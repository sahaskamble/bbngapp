    ...
    return (
      <View className="bg-gray-200 flex-1 justify-between">
        ...
        <ScrollView className="bg-white h-[80%] w-full px-4 py-4 " scrollToOverflowEnabled={true}>
          <Text className={`${isTrue ? 'text-green-500 bg-green-200' : 'text-red-500 bg-red-200'} p-2 text-center my-4`}>{isTrue ? 'True reference is Saved' : 'False reference is not saved'}</Text>
          ...
          <View className='w-full h-auto my-4 px-4 pb-4 flex flex-row'>
            <TouchableOpacity onPress={()=>{sendFormData()}} className="bg-blue-500 p-2 w-[100px] mr-2 inline-flex flex-row justify-center items-center"><Text className='text-lg'>save</Text></TouchableOpacity>
            <TouchableOpacity onPress={()=>{ setisTrue(false) }} className="bg-red-500 p-2 w-[100px] inline-flex justify-center items-center"><Text className='text-lg'>cancel</Text></TouchableOpacity>
          </View>
        </ScrollView>
      </View>
    );
